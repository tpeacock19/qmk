...
bool
process_layer_lock(uint16_t keycode, keyrecord_t *record, uint16_t lock_keycode)
{
#if LAYER_LOCK_IDLE_TIMEOUT > 0
  layer_lock_timer = timer_read32();
#endif // LAYER_LOCK_IDLE_TIMEOUT > 0
  ...
  if (keycode == lock_keycode)
    ...
    { // The layer lock key was pressed.
...
layer_lock_set_user(layer_state_t locked_layers)
{
}
